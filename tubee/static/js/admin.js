...
function load_channels(event) {
  ...
  $.ajax($("#channels-table").attr("data-template-endpoint")).done((data) => {
    channel_template = document.createElement("tr");
    channel_template.innerHTML = data;
  });
  $.getJSON(build_url("api_admin.channel")).done((data) => {
    data.forEach((element) => {
      let row = channel_template.cloneNode(true);

      row.dataset.channelId = element.id;
      row.getElementsByClassName("channel-name")[0].innerText = element.name;
      row
      ...
      row.getElementsByClassName("channel-expiration")[0].innerText =
        element.expiration;
      row.getElementsByClassName("channel-expiration-badge")[0].innerText =
        moment(element.expiration).fromNow();
      table.append(row);
    });
    init_clipboard();
