...
def channels_renew(self, channel_ids, next_countdown=-1):
    ...
    for index, channel_id in enumerate(channel_ids):
        ...
        logger.info(f"<{channel_id}> subscription renewed")
        logger.info(f"<{channel_id}> information updated")
...
def channels_refresh(channel_ids):
    ...
    for channel_id in channel_ids:
        channel = Channel.query.get(channel_id)
        if not channel:
            logger.warning(f"<{channel_id}> ID not found, skipped.")
            continue
        results[channel_id] = channel.refresh()
        logger.info(f"<{channel_id}> new hub state: {results[channel_id]['state']}")
    return results
...
def channels_fetch_videos(channel_ids):
    ...
    for channel_id in channel_ids:
        ...
        logger.info(f"<{channel_id}> videos fetched: {len(results[channel_id])}")
    return results
